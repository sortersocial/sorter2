use crate::{decode, encode, Batch, Db, DurableCollection, DurableError, Result};
    ...
    V: Serialize + for<'de> Deserialize<'de>,
{
    ...
    pub fn put_in_batch(&self, batch: &mut Batch, key: &K, value: &V) -> Result<()> {
        ...
        if is_new {
            let len_key = self.meta_key("len");
            batch.track_map_put(db_key, len_key, true);
        }
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.inner.next() {
            Some(Ok((db_key, value_bytes))) => {
                ...
                match (decode(key_bytes), decode(&value_bytes)) {
...
mod tests {
    ...
    #[test]
    fn test_put_in_batch_tracks_multiple_new_keys() {
        let (_temp, db) = setup_test_db();
        let map = DurableMap::<String, i32>::new(&db, "batch_len_map").unwrap();
        let mut batch = db.batch();

        map.put_in_batch(&mut batch, &"a".to_string(), &1).unwrap();
        map.put_in_batch(&mut batch, &"b".to_string(), &2).unwrap();
        map.put_in_batch(&mut batch, &"b".to_string(), &20).unwrap();
        map.put_in_batch(&mut batch, &"c".to_string(), &3).unwrap();
        batch.commit().unwrap();
...
mod proptests {
    ...
    fn test_nested_collections() {
        ...
        let users_posts: DurableMap<String, DurableVec<i32>> =
    ...
    fn test_nested_collection_persistence() {
        ...
        {
            let users_data: DurableMap<String, DurableVec<String>> =
