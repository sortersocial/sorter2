...
Provide ergonomic, std-like collections (`DurableMap`, `DurableVec`, `DurableSet`, …) whose contents are stored in RocksDB yet feel in-memory:

```rust
let db  = durable::open("db");               // single call opens RocksDB
let mut posts = DurableMap::<Uuid, Post>::new(&db, "posts")?;

posts.insert(id, post)?;                     // atomic single-collection write
...
2. **Typed** – Keys & values are generic; current serialization is CBOR via Serde.
3. **Atomic** – Multi-op `batch.commit()` gives RocksDB write-batch semantics.
4. **Explicit durability** – callers choose synced WAL, unsynced WAL, or disabled WAL per batch.
...
* Multi-process concurrency (one writer process is assumed)
...
Because RocksDB stores keys lexicographically, all elements of a collection (and its **nested** descendants under a parent key) live contiguously → prefix scans & prefix deletes are cheap **within that collection prefix**.

There is no general `.range(start..end)` over semantic key order: logical keys are CBOR-encoded, so lex order ≠ application sort order. Use nested collections or an app-specific raw-key index when you need parent-scoped prefix scans.
...
// DurableSet and DurableIndex are roadmap items — not implemented in v0.1.
...
* **Auto-batch**: default mutator methods create a WriteBatch, commit, and **always fsync the WAL** (`flush_wal(true)`). Durability modes apply only to explicit [`Batch::commit_with`].
* **Explicit batch**: user opens `let mut wb = db.batch();`, issues puts/deletes via collection adapters, then `wb.commit()` or `wb.commit_with(durability)` for cross-collection atomicity.
* **Crash guarantee**: after `commit_with(Durability::SyncWal)` returns, updates survive power loss (`rocksdb::DB::flush_wal(true)`). `WalOnly` and `DisableWal` are weaker by design and are **not** available on convenience `put`/`insert` methods.
...
| Operation            | Goal                                   |
| -------------------- | -------------------------------------- |
| `map.insert`         | workload-dependent; WAL fsync cost dominates `SyncWal` |
| `vec.push`           | < 25 µs                                |
